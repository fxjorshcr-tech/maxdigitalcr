...
export default function OfferSection({ data, ctaLink = "/contacto", lang = "es" }: OfferSectionProps) {
  return (
    <section id="planes" className="relative py-20 sm:py-28 md:py-36 px-4 sm:px-6 bg-neutral-50 overflow-hidden">
      <div className="pointer-events-none absolute -left-40 bottom-0 w-[600px] h-[600px] rounded-full bg-sky-400/10 blur-3xl" />
        ...
        <Reveal delay={150}>
          <div className="rounded-3xl bg-white border border-neutral-200 p-6 sm:p-8 md:p-10 flex flex-col md:flex-row md:items-center gap-8 shadow-sm">
            <div className="md:w-1/3">
              ...
              <p className="text-5xl sm:text-6xl font-bold tracking-tight text-neutral-900">{data.from}</p>
            </div>
            <p className="md:flex-1 text-neutral-600 leading-relaxed">{data.fromNote}</p>
            <Link
              href={ctaLink}
              className="group shrink-0 inline-flex items-center justify-center gap-2 px-8 py-4 rounded-full text-sm font-semibold transition-all bg-neutral-900 text-white hover:bg-[#3ECF8E] hover:text-neutral-900"
            >
              {data.cta}
        ...
        <div className="mt-8 grid grid-cols-2 lg:grid-cols-4 gap-4">
          {data.types.map((t, i) => (
            <Reveal key={t.title} delay={i * 80}>
              <div className="h-full rounded-2xl border border-neutral-200 bg-white p-5 transition-all hover:border-[#3ECF8E] hover:-translate-y-0.5">
                <div className="w-9 h-9 mb-3 rounded-lg bg-neutral-100 flex items-center justify-center text-[#2eb67d]">
