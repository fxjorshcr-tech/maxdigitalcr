...
export default function AdvisorySection({ data }: AdvisorySectionProps) {
  return (
    <section className="relative py-20 sm:py-28 md:py-36 px-4 sm:px-6 bg-white overflow-hidden">
      {/* Decorative blob */}
      <div className="pointer-events-none absolute -right-40 top-10 w-[520px] h-[520px] rounded-full bg-brand/10 blur-3xl" />
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-32">
            ...
            <Reveal delay={200}>
              <figure className="mt-10 rounded-2xl bg-neutral-950 text-white p-6 sm:p-8 relative overflow-hidden gradient-border">
                <Icon name="quote" size={40} className="absolute -top-2 -left-1 text-brand/20" />
                <blockquote className="relative text-lg leading-relaxed">{data.quote}</blockquote>
                <figcaption className="mt-4 text-sm text-brand-2 font-medium">{data.quoteBy}</figcaption>
              </figure>
            </Reveal>
          </div>
          ...
          <ol className="lg:col-span-7 space-y-5">
            {data.steps.map((step, i) => (
              <Reveal key={step.title} as="li" delay={i * 120}>
                <Spotlight className="group rounded-2xl border border-neutral-200 bg-neutral-50 p-6 sm:p-8 transition-all hover:border-brand hover:shadow-xl hover:-translate-y-0.5">
                  <div className="flex items-start gap-5">
                    <div className="relative shrink-0">
                      <div className="w-14 h-14 rounded-2xl bg-white border border-neutral-200 flex items-center justify-center text-brand-dark group-hover:bg-brand group-hover:text-neutral-900 group-hover:border-brand transition-colors">
                        <Icon name={step.icon} size={24} />
                      </div>
                      <span className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-gradient-brand text-neutral-900 text-xs font-bold flex items-center justify-center">
                        {i + 1}
                      </span>
                    </div>
                    <div>
