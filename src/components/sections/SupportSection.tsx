...
export default function SupportSection({ data }: SupportSectionProps) {
  ...
  return (
    <section className="relative py-20 sm:py-28 md:py-36 px-4 sm:px-6 bg-white overflow-hidden">
      <div className="pointer-events-none absolute -right-32 -bottom-32 w-[560px] h-[560px] rounded-full bg-lime-300/15 blur-3xl" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Reveal className="md:col-span-2 md:row-span-2">
            <Spotlight className="h-full rounded-3xl bg-neutral-950 text-white p-8 sm:p-10 flex flex-col justify-between gradient-border">
              <div>
                <div className="w-12 h-12 mb-6 rounded-xl bg-[#3ECF8E]/15 flex items-center justify-center text-[#3ECF8E]">
          ...
          {rest.map((item, i) => (
            <Reveal key={item.title} delay={(i + 1) * 80}>
              <Spotlight className="h-full rounded-3xl border border-neutral-200 bg-neutral-50 p-6 transition-all hover:border-[#3ECF8E] hover:bg-white hover:shadow-lg">
                <div className="w-10 h-10 mb-4 rounded-lg bg-white border border-neutral-200 flex items-center justify-center text-[#2eb67d]">
