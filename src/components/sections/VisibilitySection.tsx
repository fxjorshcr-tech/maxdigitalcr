...
export default function VisibilitySection({ data, lang = "es" }: VisibilitySectionProps) {
  ...
  return (
    <section className="relative py-20 sm:py-28 md:py-36 px-4 sm:px-6 bg-neutral-950 text-white overflow-hidden grain">
      <div className="pointer-events-none absolute left-1/2 top-0 -translate-x-1/2 w-[900px] h-[500px] rounded-full bg-[#3ECF8E]/10 blur-3xl" />
      ...
      <div className="relative max-w-6xl mx-auto">
            ...
            <div className="relative">
              <div className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur p-6 gradient-border">
                <div className="flex items-center gap-2 text-xs text-neutral-400 mb-4">
                  <Icon name="sparkles" size={14} className="text-[#3ECF8E]" />
                  <span className="font-mono">{mock.query}</span>
                </div>
                <p className="text-sm text-neutral-300 leading-relaxed">
                  {mock.ai}{" "}
                  <span className="inline-flex items-center gap-1 rounded-md bg-[#3ECF8E]/15 border border-[#3ECF8E]/40 px-1.5 py-0.5 text-[#3ECF8E] font-semibold">
                    {mock.biz}
                ...
                <div className="mt-5 rounded-xl bg-neutral-900 border border-white/10 p-4 flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-[#3ECF8E]/15 flex items-center justify-center text-[#3ECF8E]">
                  ...
                  <div className="ml-auto flex gap-0.5 text-[#3ECF8E]">
                    {[0, 1, 2, 3, 4].map((i) => (
              ...
              <div className="float-slow absolute -right-3 -top-6 sm:-right-8 rounded-full border border-white/10 bg-neutral-900/90 backdrop-blur px-4 py-2 text-xs text-neutral-300 shadow-xl flex items-center gap-2">
                <Icon name="compass" size={14} className="text-[#3ECF8E]" />
                {mock.siri}
              </div>
              <div className="float-slow [animation-delay:-3s] absolute -left-3 -bottom-6 sm:-left-8 rounded-full border border-white/10 bg-neutral-900/90 backdrop-blur px-4 py-2 text-xs text-neutral-300 shadow-xl flex items-center gap-2">
                <Icon name="globe" size={14} className="text-[#3ECF8E]" />
                {mock.bing}
              </div>
            </div>
          </Reveal>
          ...
          <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {data.items.map((item, i) => (
              <Reveal key={item.title} delay={i * 80}>
                <Spotlight className="h-full rounded-2xl border border-white/10 bg-white/[0.03] p-5 sm:p-6 transition-all hover:border-[#3ECF8E]/50 hover:bg-white/[0.06]">
                  <div className="w-10 h-10 mb-4 rounded-lg bg-[#3ECF8E]/15 flex items-center justify-center text-[#3ECF8E]">
        ...
        <Reveal delay={200}>
          <p className="mt-12 text-center text-sm text-neutral-500 flex items-center justify-center gap-2">
            <Icon name="shield" size={16} className="text-[#3ECF8E]" />
            {data.footnote}
          </p>
        </Reveal>
      </div>
